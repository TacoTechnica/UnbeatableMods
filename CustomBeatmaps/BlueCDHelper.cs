...
namespace CustomBeatmaps
{
    ...
    public static class BlueCDHelper
    {
        ...
        private static Color PAUSE_MENU_CUSTOM_RECORD_COLOR = new Color(0.2901961f, 0.6709353f, 0.9999f, 1);
        private static Material _alternativeTextMaterial = null;
        ...
        public static void ApplyBlueCD()
        {
            // Add some color to pause menu
                ...
                if (pauseMenu != null)
                {
                    Transform record = pauseMenu.transform.GetChild(0);

                    // Set back color
                    Transform back = pauseMenu.transform.parent.GetChild(0);
                    Image backImage = back.GetComponent<Image>();
                    if (backImage != null)
                    {
                        backImage.color = PAUSE_MENU_CUSTOM_RECORD_COLOR;
                    }

                    // Set text outline color
                    foreach (TMP_Text menuText in pauseMenu.transform.GetComponentsInChildren<TMP_Text>(true))
                    {
                    ...
                    // Set record color
                    Image recordImage = record.GetComponent<Image>();
